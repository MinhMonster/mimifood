    ...
    protected function rules(?int $id = null): array
    {
        return [
            ...
            'username' => [
                'required',
                'string',
                Rule::unique('ninjas')->ignore($id)->whereNull('deleted_at'),
            ],
            ...
            ...collect(range(1, 12))->mapWithKeys(fn ($i) => ["tl_$i" => 'nullable|integer|max:9'])->all(),
            ...collect(range(1, 13))->mapWithKeys(fn ($i) => ["item_$i" => 'nullable|string'])->all(),
        ];
    }
}
